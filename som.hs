...
vUpdate img pixel index = let index' = index*3
                              ...
                              (iup,up)= if' (index>=50) (index-50) 2
                              (iup',up')= if' (index>=100) (index-100) 10
                              (idown,down)= if' (index<=2449) (index+50) 2
                              (idown',down')= if' (index<=2399) (index+100) 10
                              (iright,right)= if' (mod' 49) (index+1) 2
                              (iright',right')= if' ((mod' 49)&&(mod' 48)) (index+2) 10
                              (ileft,left)= if' (mod' 0) (index-1) 2
                              (ileft',left')= if' ((mod' 0)&&(mod' 1)) (index-2) 10
                              (iupright,upright)= if' (index>=50 && mod' 49) (index-49) 5
                              (iupleft,upleft)= if' (index>=50 && mod' 0) (index-51) 5
                              (idownright,downright)= if' (index<=2449 && mod' 49) (index+51) 5
                              (idownleft,downleft)= if' (index<=2449 && mod' 0) (index+49) 5
...
mainloop _ _ img = do let leng = V.length img 
                      if leng == (7500)
                        then do putStr "STOP" ; hFlush stdout; tex <- getLine; print tex;return ()
                        else do print $(leng - 7500); return ()
                      let (img', file) = V.splitAt (7500) img
